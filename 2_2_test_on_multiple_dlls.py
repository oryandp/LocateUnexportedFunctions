...
def run_script_for_all_ntdlls(script_path, ntdll_files, function_name, dll_name='ntdll', from_call=False):
	output_dir = os.path.join(SCRIPTS_DIR, 'Output')
	
	# Create output files with headers
	if from_call:	
		output_file = '%s_%s_by_byteseq_from_call.csv' % (dll_name, function_name)	   
	else:
	...
	for ntdll_path, win_version, is_x64, byte_seq, offset in ntdll_files:
		...
		subprocess.call([ida_path, '-A', '-S"{}" "{}" "{}" "{}" "{}" "{}" "{}" "{}"'.format(script_path, win_version, is_x64, function_name, byte_seq, offset, output_path, from_call), ntdll_path])
...
def main():
	...
	run_script_for_all_ntdlls(script_to_run, ntdll_files, 'LdrpHandleTlsData', from_call=from_call)
	print("Done!")
